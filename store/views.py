    ...
    if request.user.is_authenticated:
        ...
        if request.method == "POST":
            ...
            if auth is not None:
                ...
                messages.error(request, 'Колдонуучу аты менен сырсөз дал келбейт')
    ...
    if request.method == 'POST':
        if request.user.is_authenticated:
            if form.is_valid():
                ...
                messages.success(request, "Сын-пикир ийгиликтүү кошулду")
                form = ReviewForm()
        else:
            messages.error(request, "Сиз биринчи логин керек.")
    context = {
        "book": book,
        "rbooks": rbooks,
        "form": form,
